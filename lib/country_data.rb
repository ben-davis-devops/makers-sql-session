...
class CountryData
  ...
  def self.population_density_between_50_and_150
    result = DatabaseConnection.query("SELECT * FROM countries WHERE ((density < 150) AND (density > 50)) ORDER BY density;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
  ...
  def self.third_lowest_gni_per_capita_in_south_america
    result = DatabaseConnection.query("SELECT * FROM countries WHERE continent = 'Asia' ORDER BY -gni DESC LIMIT 1 OFFSET 2;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end

  def self.all_countries_not_in_europe
    result = DatabaseConnection.query("SELECT * FROM countries WHERE continent != 'Europe' ORDER BY continent;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end

  def self.all_data_country_name_and_population_only
    result = DatabaseConnection.query("SELECT name, population FROM countries WHERE continent = 'South America' ORDER BY name;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end

  def self.south_american_countries_hide_population
    result = DatabaseConnection.query("SELECT name, continent, density, gni FROM countries WHERE continent = 'South America' ORDER BY name;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end

  def self.countries_not_in_asia_hide_gni_per_capita_and_population_density
    result = DatabaseConnection.query("SELECT name, continent, population FROM countries WHERE continent != 'Asia' ORDER BY continent;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
end
